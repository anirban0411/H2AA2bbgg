...
void H2AA2bbgg_analysis_treeread_18_v1()
{
	...
	double ZH_mA20 = (59730*0.001)/(909749);
        double ZH_mA25 = (59730*0.001)/(942756);
        double ZH_mA30 = (59730*0.001)/(990761);
        double ZH_mA35 = (59730*0.001)/(777789);
        double ZH_mA40 = (59730*0.001)/(599839);
        double ZH_mA45 = (59730*0.001)/(645819);
        double ZH_mA50 = (59730*0.001)/(732824);
        double ZH_mA55 = (59730*0.001)/(857810);
        double ZH_mA60 = (59730*0.001)/(888778);
    ...
    TFile *f1 = new TFile("/home/abala/local_bkp/analysis/root_file/ZH_2018/V1/ZH_mA60_v2.root");

    TTree *Tout1 = (TTree*)f1->Get("Tout");

    TFile *fout = new TFile("output_ZHToAA2B2G_M60_pythia8_ggh.root","RECREATE");

    TTree *T0 = new TTree("ggh_60_13TeV_H2AA2bbgg_Tag0", "ggh_60_13TeV_H2AA2bbgg_Tag0");
		...
		if((isEle || isMu) && (bb == 1) && (weight_nom > 0)){
				...
				float chi2 = (((bbgg_invmass - 125)/sigmah1)*((bbgg_invmass - 125)/sigmah1) + ((bb_invmass - gg_invmass)/sigmabb1)*((bb_invmass - gg_invmass)/sigmabb1));
                    		...
                    		float chi2_resdn = (((bbgg_invmass_resdn - 125)/sigmah1)*((bbgg_invmass_resdn - 125)/sigmah1) + ((bb_invmass_resdn - gg_invmass)/sigmabb1)*((bb_invmass_resdn - gg_invmass)/sigmabb1));
                                ...
                                if (chi2 <= 5 /*&& delphi_bb <= M_PI/2 && delphi_gg <= M_PI/2*/) {

				if (gg_invmass >= 15 && gg_invmass <= 70) {
				CMS_hgg_mass = gg_invmass;
				weight = (weight_nom * 880 * 0.001)/num;          
				dZ = 0.0;
				...
				if (gg_invmass >= 15 && gg_invmass <= 70) {
                                CMS_hgg_mass = gg_invmass;
                                weight = (weight_PU_up * 880 * 0.001)/num;
                                dZ = 0.0;
                                sum_PU_up = sum_PU_up + weight; }

				if (gg_invmass >= 15 && gg_invmass <= 70) {
                                CMS_hgg_mass = gg_invmass;
                                weight = (weight_leptonsf_up * 880 * 0.001)/num;
                                dZ = 0.0;
                                sum_leptonsf_up = sum_leptonsf_up + weight; }

				if (gg_invmass >= 15 && gg_invmass <= 70) {
                                CMS_hgg_mass = gg_invmass;
                                weight = (weight_photonsf_up * 880 * 0.001)/num;
                                dZ = 0.0;
                                sum_photonsf_up = sum_photonsf_up + weight; }

				if (gg_invmass >= 15 && gg_invmass <= 70) {
                                CMS_hgg_mass = gg_invmass;
                                weight = (weight_bSF_up * 880 * 0.001)/num;
                                dZ = 0.0;
				sum_bSF_up = sum_bSF_up + weight; }

				if (gg_invmass >= 15 && gg_invmass <= 70) {
                                CMS_hgg_mass = gg_invmass;
                                weight = (weight_trig_up * 880 * 0.001)/num;
                                dZ = 0.0;
				sum_trig_up = sum_trig_up + weight; }

				if (gg_invmass >= 15 && gg_invmass <= 70) {
                                CMS_hgg_mass = gg_invmass;
                                weight = (weight_pujid_up * 880 * 0.001)/num;
                                dZ = 0.0;
				sum_pujid_up = sum_pujid_up + weight; }

				if (gg_invmass >= 15 && gg_invmass <= 70) {
                                CMS_hgg_mass = gg_invmass;
                                weight = (weight_PU_dn * 880 * 0.001)/num;
                                dZ = 0.0;
                                sum_PU_dn = sum_PU_dn + weight; }

				if (gg_invmass >= 15 && gg_invmass <= 70) {
                                CMS_hgg_mass = gg_invmass;
                                weight = (weight_leptonsf_dn * 880 * 0.001)/num;
                                dZ = 0.0;
                                sum_leptonsf_dn = sum_leptonsf_dn + weight; }

                                if (gg_invmass >= 15 && gg_invmass <= 70) {
                                CMS_hgg_mass = gg_invmass;
                                weight = (weight_photonsf_dn * 880 * 0.001)/num;
                                dZ = 0.0;
                                sum_photonsf_dn = sum_photonsf_dn + weight; }

				if (gg_invmass >= 15 && gg_invmass <= 70) {
                                CMS_hgg_mass = gg_invmass;
                                weight = (weight_bSF_dn * 880 * 0.001)/num;
                                dZ = 0.0;
				sum_bSF_dn = sum_bSF_dn + weight; }

				if (gg_invmass >= 15 && gg_invmass <= 70) {
                                CMS_hgg_mass = gg_invmass;
                                weight = (weight_trig_dn * 880 * 0.001)/num;
                                dZ = 0.0;
				sum_trig_dn = sum_trig_dn + weight; }

				if (gg_invmass >= 15 && gg_invmass <= 70) {
                                CMS_hgg_mass = gg_invmass;
                                weight = (weight_pujid_dn * 880 * 0.001)/num;
                                dZ = 0.0;
