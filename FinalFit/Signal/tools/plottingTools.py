...
def plotFTest(ssfs,_opt=1,_outdir='./',_extension='',_proc='',_cat='',_mass='20,25,30,35,40,45,50,55,60'):
  canv = ROOT.TCanvas()
...
#    hists[k].GetXaxis().SetRangeUser(115,140)
    hists[k].GetXaxis().SetRangeUser(15,70)
...
#  hists['data'].GetXaxis().SetRangeUser(115,140)
  hists['data'].GetXaxis().SetRangeUser(15,70)
...
def plotFTestResults(ssfs,_opt,_outdir="./",_extension='',_proc='',_cat='',_mass='20,25,30,35,40,45,50,55,60'):
  canv = ROOT.TCanvas()
  gr = ROOT.TGraph()
...
#  hists['final'].GetXaxis().SetRangeUser(100,150)
  hists['final'].GetXaxis().SetRangeUser(15,70)
...
def plotInterpolation(_finalModel,_outdir='./',_massPoints='20,22,25,27,30,32,35,37,40,42,45,47,50,52,55,57,60'):

  canv = ROOT.TCanvas()
  colors = [ROOT.kRed,ROOT.kCyan,ROOT.kBlue+1,ROOT.kOrange-3,ROOT.kRed+3,ROOT.kMagenta-7,ROOT.kGreen+1,ROOT.kBlue+3,ROOT.kYellow-7,ROOT.kViolet+6,ROOT.kTeal+1,ROOT.kPink+1,ROOT.kYellow+3,ROOT.kAzure+1,ROOT.kGreen+3,ROOT.kCyan+3,ROOT.kBlue+6]
  colorMap = {}
  ...
  for mp in _massPoints.split(","):
    ...
    print("Integral: {}".format(hists[mp].Integral()))
#    print("Bins: {}".format(_finalModel.xvar.getBins()))
    if norm == 0.: 
      hists[mp].Scale(0.)
    else:
  ...
  haxes.SetMinimum(0)
  haxes.SetMaximum(hmax*1.5)        #axes for simultaneous fit plots 
#  haxes.SetMaximum(3.0)
#  haxes.GetXaxis().SetRangeUser(100,150)
  haxes.GetXaxis().SetRangeUser(15,70)
  haxes.Draw("AXIS")
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Plot splines
def plotSplines(_finalModel,_outdir="./",_nominalMass='20,25,30,35,40,45,50,55,60',splinesToPlot=['xs','br','ea','fracRV']):
  canv = ROOT.TCanvas()
  ...
  haxes.GetXaxis().SetTitleOffset(0.85)
  haxes.GetXaxis().SetLabelSize(0.035)
  haxes.GetYaxis().SetTitle("X/X(m_{H})")
  haxes.GetYaxis().SetTitleOffset(0.85)
  haxes.GetYaxis().SetTitleSize(0.05)
...
#  h_axes.GetXaxis().SetRangeUser(105,140)
  h_axes.GetXaxis().SetRangeUser(15,70)
  h_axes.SetTitle("")
