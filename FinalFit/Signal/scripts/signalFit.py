...
#  parser.add_option('--massPoints', dest='massPoints', default='120,125,130', help="Mass points to fit")
  parser.add_option('--massPoints', dest='massPoints', default='20,25,30,35,40,45,50,55,60', help="Mass points to fit")
...
if not opt.skipVertexScenarioSplit:
  ...
  print datasetWVForFit[MHNominal].sumEntries()
...
#    procReplacementFit, catReplacementFit = rMap['procWVMap'][opt.cat], rMap['catWVMap'][opt.cat]
    for mp in opt.massPoints.split(","):
      print procReplacementFit
      WSFileName = glob.glob("%s/output*M%s*%s.root"%(opt.inputWSDir,mp,procReplacementFit))[0]
      f = ROOT.TFile(WSFileName,"read")
