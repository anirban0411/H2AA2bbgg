...
mgg = ROOT.RooRealVar(opt.xvar,opt.xvar,20)
...
def getHistograms( _ws, _nominalDataName, _sname ):
  ...
  print "Printing ws "
  _ws.Print()
  print "nomDName" , _nominalDataName
  print "syst name" , _sname
  # Define histograms
  for htype in ['nominal','up','down']:
    ...
    if htype == 'nominal': _hists[htype] = ROOT.TH1F(htype,htype,opt.nBins,15,70)
    else: _hists[htype] = ROOT.TH1F("%s_%s"%(_sname,htype),"%s_%s"%(_sname,htype),opt.nBins,15,70)
...
for _proc in opt.procs.split(","):
  # Glob M125 filename
  _WSFileName = glob.glob("%s/output*M20*%s.root"%(opt.inputWSDir,_proc))[0]
  _nominalDataName = "%s_20_%s_%s"%(procToData(_proc.split("_")[0]),sqrts__,opt.cat)
