...
files = [f'output_VHToAA2B2G_M{mass}_pythia8_ggh.root' for mass in original_mass_points]
...
for file, mass in zip(files, original_mass_points):
    with uproot.open(file) as f:
        tree_name = f"ggh_{mass}_13TeV_AC_Tag0"
        tree = f[tree_name]
        x_data = tree["CMS_hgg_mass"].array(library="np")
        weights = tree["weight"].array(library="np")

    hist, bin_edges = np.histogram(x_data, bins=800, range=(15, 70), weights=weights)
